...
def instance_bce_with_logits(logits, labels, reduction='mean'):
    ...
    # print("instance_bce_with_logits.loss:", loss)
    # print("labels.size(1):", labels.size(1))
    if reduction == 'mean':
        loss *= labels.size(1)
    return loss
...
def compute_self_loss(logits_neg, a):
    ...
    # print("neg_top_k.shape:", neg_top_k.shape)
...
def train(model, train_loader, eval_loader, opt):
    ...
    optim = get_optim(opt, model, data_size=438183)
    ...
    for epoch in range(opt.s_epoch, opt.num_epochs):
        ...
        N = len(train_loader.dataset)
                    # Learning Rate Decay
        if epoch in opt.LD_DECAY_LIST:
            adjust_lr(optim, opt.LD_DECAY_R)
        # scheduler.step()

        for i, (v, b, q, a,_) in enumerate(train_loader):
            v = v.cuda() # torch.Size([256, 36, 2048])
            q = q.cuda() # torch.Size([256, 14])
            a = a.cuda() # torch.Size([256, 2274])

            # for the labeled samples
            if epoch < opt.pretrain_epoches:
                logits_pos, _= model(q, v, False) # torch.Size([256, 2274])
                if opt.ml_loss:
